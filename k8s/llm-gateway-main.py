    ...
    with tracer.start_as_current_span("llm.ollama_chat") as span:
        ...
        async with httpx.AsyncClient(timeout=OLLAMA_TIMEOUT_SECONDS) as client:
            try:
                ...
                raise HTTPException(
                    status_code=504,
                    detail="Ollama demorou para responder. Aguarde o modelo terminar de gerar ou tente uma pergunta menor.",
                )
                ...
                span.record_exception(exc)
                raise HTTPException(status_code=503, detail=f"Ollama request failed: {exc.__class__.__name__}")
